...
#Load Packages
list.of.packages <- c("leaflet", "dplyr", "viridis", "kriging", "corrplot", "lubridate", "kableExtra", "purrr", "tidyr", "hms")
...
# Updated to use ggplot Oct 2024 to speed up plotting
...
{
  eff_tmp <- dplyr::filter(eff, Plot.grp == j)
  dat_tmp <- filter(dat, Deployment.Location.ID %in%
                      unique(eff_tmp$Deployment.Location.ID))
  
  species_activty_check <- ggplot() +
    geom_segment(data = eff_tmp,
                 aes(x = ymd_hms(paste0(Camera.Deployment.Begin.Date, 
                                        " 12:00:00")),
                     xend = ymd_hms(paste0(Camera.Deployment.End.Date, 
                                           " 12:00:00")),
                     y = Deployment.Location.ID,
                     yend = Deployment.Location.ID)) +
    geom_point(data = dat_tmp, 
               aes(x = ymd_hms(Date_Time.Captured),
                   y = Deployment.Location.ID,
                   colour = Species),
               shape = "|") +
...
```{r}
# updated to use ggplot Jan 2024

sp.order <- sort(unique(dat$Species))
plot_grp <- data.frame("Species"= sp.order, "Plot.grp"=ceiling(1:length(unique(dat$Species))/20))

tmp4 <- left_join(dat, plot_grp, by = "Species") %>% 
  mutate(time_of_day = hms::as_hms(ymd_hms(Date_Time.Captured)))

j <- 1
for(j in 1:length(unique(tmp4$Plot.grp)))
{
  tmp <- filter(tmp4, Plot.grp == j)
  
  plot <- ggplot(tmp, aes(x = time_of_day, y = Species)) +
    geom_vline(xintercept = c(hms::as_hms("06:00:00"), hms::as_hms("18:00:00")),
               colour = "grey", linetype = "dashed") +
    geom_point(alpha = 0.7, shape = 1, colour = "#20B2AA20") +
    scale_x_time(name = "", 
                 breaks = c(hms::as_hms("00:00:00"), hms::as_hms("06:00:00"),
                            hms::as_hms("12:00:00"), hms::as_hms("18:00:00"),
                            hms::as_hms("24:00:00")),
                 labels=c("00:00", "06:00", "12:00", "18:00", "24:00")) +
    theme(panel.grid.major.y = element_line(size=.1, color="lightgrey"),
...
The summary of detections after excluding humans is as follows:

```{r ind captures no humans, echo=F, fig.height=sp.height, eval=T}
if(plot_without_humans == TRUE){

ind.dat_no_humans <- filter(ind.dat, Species != "Homo sapiens")

layout(matrix(c(1,1,2), 1, 3, byrow = TRUE))
det.sum.total <- as.data.frame(count(ind.dat_no_humans[ind.dat_no_humans$Blank==FALSE,], Species))
det.sum.total <- det.sum.total[order(det.sum.total$n),]
