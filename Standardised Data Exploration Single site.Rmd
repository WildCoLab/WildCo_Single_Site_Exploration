...
event_grp <- dat %>% 
  ...
  mutate(if_else(Sex == "", "unknown", Sex),
         if_else(Age == "", "unknown", Age)) %>% 
...
### NOTE - the total count of individuals in this data frame may differ slightly from that in ind.dat if there are gaps greater than your independence threshold between detections of the same age and sex class, but not in the species as a whole.
...
# Remove observations without animals detected
dat_sa <- dat_sa[dat_sa$Blank==FALSE & is.na(dat_sa$Species)==FALSE,]
dat_sa$Species <- as.character(dat_sa$Species)
dat_sa$Deployment.Location.ID <- as.character(dat_sa$Deployment.Location.ID)

# Order the datframe by Site, date
dat_sa <- dat_sa[order(dat_sa$Deployment.Location.ID, dat_sa$Date_Time.Captured),]
...
dat_sa <- dat_sa %>%
  group_nest(Deployment.Location.ID, Species, Age, Sex) %>%
  mutate(data = map(.x = data, 
                    .f =~ func2(.x))) %>%
  unnest(data) %>% 
  #need Event.IDs to all be unique:
  mutate(Event.ID = paste(Event.ID, Deployment.Location.ID, 
                          Species, Sex, Age, sep = "_"))

# If there is no minimum groupsize take number of animals
if(!"Minimum.Group.Size" %in% colnames(dat_sa)) {dat_sa$Minimum.Group.Size <- dat_sa$Number.of.Animals}

# Calculate the event length and size

# find out the last and the first of the time in the group
top <- dat_sa %>% group_by(Event.ID) %>% top_n(1,Date_Time.Captured) %>% select(Event.ID, Date_Time.Captured)
bot <- dat_sa %>% group_by(Event.ID) %>% top_n(-1,Date_Time.Captured) %>% select(Event.ID, Date_Time.Captured)
names(bot)[2] <- c("Date_Time.Captured_end")
dec_no <- dat_sa %>% group_by(Event.ID) %>% summarise(n())

### event group size
event_grp <- dat_sa %>% 
  group_by(Event.ID) %>%
  summarise(Event.Groupsize = max(Minimum.Group.Size))
...
# Subset to the first observation in each event
ind.dat_sa <- dat_sa[!duplicated(dat_sa$Event.ID),]
ind.dat_sa <- as.data.frame(ind.dat_sa)
