...
# Week -year format
  ...
  tmp$Date <- strftime(tmp$Date, format = "%Y-W%V")
  ...
  for(i in 1:nrow(week.obs))
    {
      tmp <- ind.dat[ind.dat$Deployment.Location.ID==week.obs$Deployment.Location.ID[i] & strftime(ind.dat$Date_Time.Captured, format = "%Y-W%V")== week.obs$Date[i],]
