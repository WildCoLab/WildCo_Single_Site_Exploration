...
tmp4 <- data.frame("Species"=sp.order, "Plot.grp"=ceiling(1:length(unique(dat$Species))/20))
...
for(j in 1:length(unique(tmp4$Plot.grp)))
{
    ...
    plot(c(0,24),c(1,length(unique(dat$Species[tmp4$Plot.grp==j]))), las=1, ylab="", xlab="", type="n", yaxt="n", xaxt="n")
    axis(1, at=c(0,6,12,18,24), labels=c("00:00", "06:00", "12:00", "18:00", "24:00"))
    
    axis(2, at= 1:length(unique(dat$Species[tmp4$Plot.grp==j])), labels= unique(tmp4$Species[tmp4$Plot.grp==j]), las=1, cex.axis=1)
